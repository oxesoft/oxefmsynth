...
class CClapHostInterface : public CHostInterface
{
public:
    explicit CClapHostInterface(COxeClapPlugin *plugin);
    virtual ~CClapHostInterface() = default;

    void ReceiveMessageFromPlugin(unsigned int messageID, unsigned int par1, unsigned int par2) override;

private:
    COxeClapPlugin *plugin;
};
